...
export const complexityAccessor = <T extends KeySkill>(config: {
  ...
  header: () => (
    <div className="flex items-center gap-1">
      <div>{config.header ?? 'Complexity'}</div>
      <div>
        <ComplexityTooltip />
      </div>
    </div>
...
export const prevPlaceAccessor = <T extends KeySkill>(
  ...
  cell: info => {
    ...
    return (
      <ValueChangeRenderer prev={prev} current={current ?? 0} inverse={true} />
    );
  },
  size: 50,
  enablePinning: true,
  ...
  cell: info => {
    return (
      <CategoryDescription
        ...
        category={info.row.original as Category}
        image={false}
      />
    );
  },
      ...
      <>
        {confidence ? (
          <div className="flex items-center gap-1">
            ...
            <div className="w-10 truncate text-sm">
